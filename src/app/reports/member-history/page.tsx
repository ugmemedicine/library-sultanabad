...
import { getMemberHistoryReport } from "@/services/reportService";
...
export default function MemberHistoryReportPage() {
  ...
  const loadData = async (nextFilters: ReportFilterFormState) => {
    ...
    setLoans(await getMemberHistoryReport(request));
  };
  ...
  const handleExport = () => {
    ...
    downloadCsv("member-history-report.csv", toCsv(rows));
  };
  ...
  return (
    <ProtectedRoute>
      <RoleGate roles={["admin", "librarian"]}>
        <PageHeader title="Member History Report" text="Loan history records across members with date and text filtering." />
        <ReportFiltersBar
          filters={filters}
          onChange={setFilters}
